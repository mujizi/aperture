...
- 编号流程、对比矩阵、指标卡片、小图标和少量语义颜色保留；普通 `done` 状态不再显示，只有 `partial`、`proposed`、`unverified` 等例外状态保留标签；
...
当前基线仍不是最终设计。它的优点是比回退前的实验更自然、层级更容易识别，且窄窗口下文字可读性已有改善；缺点是例外状态标签、图标、彩色短语和指标卡片仍可能产生噪声，模型也可能把并列信息误判为流程或对比。
...
| 当前聚焦度 | `0.30` |
| 最近验证 | TypeScript、34 项测试、Web/Server/原生应用构建、插件校验、应用健康检查、Computer Use 实际窗口检查通过 |
...
| Highlight | 短语底色和细下划线 | 标记 key、change、decision、risk、verified |
...
- 高亮短语必须原样存在于文本中，重复高亮删除，最多保留 3 个；
